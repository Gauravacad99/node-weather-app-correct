...
const forecast = function (longitude, latitude, callback) {
    ...
    request({ url: url, json: true }, (error, response) => {
        if (error) {
            ...
            callback(undefined, `It is currently ${response.body.current.temperature} degrees out. It feels like ${response.body.current.feelslike} degrees out. The humidity is ${response.body.current.humidity}`)
